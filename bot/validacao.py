...
def verificar_observacao_reprovado(status : str, observacao: str):
    ...
    # Padroniza o texto para evitar erros de maiúsculas/minúsculas.
    status_normalizado = str(status).strip().upper() if status else ""

    if status_normalizado == 'REPROVADO':
        observacao_normalizada = str(observacao).strip().lower()
        observacao_ausente = observacao is None or observacao_normalizada in {"", "nan", "<na>"}
        if observacao_ausente:
